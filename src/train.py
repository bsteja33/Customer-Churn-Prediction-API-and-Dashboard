...
def load_data(config: Dict[str, Any]) -> pl.DataFrame:
    """
    Stream data from Hugging Face or load from a local CSV.

    :param config: Dictionary containing data configuration.
    :type config: dict
    :return: Polars DataFrame containing the raw dataset.
    :rtype: pl.DataFrame
    """
...
def _clean_features(dataframe: pl.DataFrame, config: Dict[str, Any]) -> pl.DataFrame:
    """
    Clean columns and handle missing values before encoding.
...
def _encode_features(dataframe: pl.DataFrame) -> Tuple[pl.DataFrame, pl.Series]:
    """
    Apply categorical encodings and extract target labels.

    :param dataframe: The cleaned Polars DataFrame.
    :type dataframe: pl.DataFrame
    :return: Tuple of feature matrix X and target vector y.
    :rtype: Tuple[pl.DataFrame, pl.Series]
    """
...
def engineer_features(dataframe: pl.DataFrame, config: Dict[str, Any]) -> Tuple[pl.DataFrame, pl.Series]:
    """
    Pure Polars feature engineering orchestrator.
...
def _train_model(
    ...
    :param y_test: Validation target.
    :type y_test: pl.Series
    :param seed: Random seed.
    :type seed: int
    :return: Trained LGBMClassifier.
