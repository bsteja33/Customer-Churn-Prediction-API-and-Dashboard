...
from src.data_preprocessing import _load_local_csv  # noqa: E402
...
class TestDataPreprocessing:
    ...
    def test_load_hf_dataset(self, mock_load_dataset):
        from src.data_preprocessing import _load_hf_dataset
        mock_load_dataset.return_value = [{"feature": 1}, {"feature": 2}]
        config = {"dataset": "fake-dataset"}
        df = _load_hf_dataset(config)
        assert len(df) == 2
        mock_load_dataset.assert_called_once_with("fake-dataset", split="train", streaming=True)

    def test_load_local_csv_missing(self):
        from src.data_preprocessing import _load_local_csv
        config = {"raw_path": "does_not_exist.csv"}
        with pytest.raises(FileNotFoundError):
            _load_local_csv(config)

    @patch("src.data_preprocessing._load_hf_dataset")
    def test_load_and_clean_hf(self, mock_hf):
        ...
        from src.data_preprocessing import DATA_CONFIG
        DATA_CONFIG["raw_path"] = None
        DATA_CONFIG["drop_columns"] = ["DropMe"]
        DATA_CONFIG["target_column"] = "Churn"
        DATA_CONFIG["positive_class"] = 1
        DATA_CONFIG["negative_class"] = 0
        df = load_and_clean()
